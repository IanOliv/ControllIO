...
class Files  {
    ...
    constructor({ name, hasItsOwnEvents, hasItsOwnJsFile}){
        ...
        let existsDir =this.dirAlreadyExists()
    ...
    createJsFile(hasEventFile){
        try {
            let contentFile = this.templateFiles.getJsFile(this.name, hasEventFile)
    ...
    createEvents(){
        ...
        try{
            let contentFile = this.templateFiles.getEventFile(this.name)
